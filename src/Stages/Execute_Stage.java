...
public class Execute_Stage {
    ...
    public static Queue<Object> executionQ = new LinkedList<>();
    public static int BranchAddressResult;

    public static void Execute(String ALUOp, boolean ALUSrc, boolean MemUse, String SignExtend, int pcPlus2, String instruction,
                               boolean MemRead, boolean MemWrite, boolean Branch, boolean MemToReg, boolean RegWrite, boolean Jump,
                               int programSize, boolean RegDst) {

        int SignExtendInt = getTwosComplement(SignExtend);
        if (MemUse)
            ALU.evaluate(ALUOp,RegisterFile.registers[0],SignExtendInt);
        else {
            if (ALUSrc)
                ALU.evaluate(ALUOp,RegisterFile.registers[Integer.parseInt(instruction.substring(4,8),2)],SignExtendInt);
            else
                ALU.evaluate(ALUOp,RegisterFile.registers[Integer.parseInt(instruction.substring(4,8),2)],RegisterFile.registers[Integer.parseInt(instruction.substring(8,12),2)]);
        }
        BranchAddressResult = pcPlus2 + (SignExtendInt*2);
        System.out.println(convertInstruction(instruction) + " in Execute stage:" + "\n");
        if (ALU.Z_Flag)
            System.out.println("zero flag -> 1");
        else
            System.out.println("ZeroFlag -> 0");
        System.out.println("branch address -> " + convertBin(BranchAddressResult));
        System.out.println("ALU result/address -> " + convertBin(ALU.output));
        System.out.println("register value to write to memory -> " + convertBin(RegisterFile.registers[Integer.parseInt(instruction.substring(4,8),2)]));
        ...
        MemoryAccess_Stage.memAccQ.add(ALU.output);
        MemoryAccess_Stage.memAccQ.add(ALU.Z_Flag);
        MemoryAccess_Stage.memAccQ.add(BranchAddressResult);
        MemoryAccess_Stage.memAccQ.add(MemWrite);
        MemoryAccess_Stage.memAccQ.add(instruction);
        MemoryAccess_Stage.memAccQ.add(MemRead);
        MemoryAccess_Stage.memAccQ.add(Branch);
        MemoryAccess_Stage.memAccQ.add(Jump);
        MemoryAccess_Stage.memAccQ.add(MemToReg);
        MemoryAccess_Stage.memAccQ.add(RegWrite);
        MemoryAccess_Stage.memAccQ.add(programSize);
        MemoryAccess_Stage.memAccQ.add(RegDst);
    }
    ...
    public static String findTwoscomplement(StringBuffer str)  //converts binary to 2's complement
    {
        int n = str.length();
        int i;
        for (i = n-1 ; i >= 0 ; i--)
